...
from utils.message import errmsg
...
def parse_pagination(query):
    page = request.args.get("page", type=int, default=CONST.DEFAULT_PAGE)
    per_page = min(
        request.args.get("per_page", type=int, default=CONST.DEFAULT_PER_PAGE), 1000
    )
